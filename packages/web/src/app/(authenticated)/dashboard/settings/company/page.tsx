...
import { Building2, Save, Users, Link, Copy, Check, Plus, Trash2, UserMinus, Mail, Calendar, BarChart3 } from 'lucide-react'
...
export default function CompanySettingsPage() {
  ...
  // Fetch company data
  const { data: company, isLoading, refetch } = api.company.getMyCompany.useQuery()
  const { data: inviteLinks = [], refetch: refetchLinks } = api.company.getInviteLinks.useQuery()
  
  // Fetch company members and stats
  const { data: membersData, refetch: refetchMembers } = api.company.getCompanyMembers.useQuery(
    ...
    try {
      if (!company) {
        ...
        await updateSharedData.mutateAsync({
          companyId: company.id,
          data: Object.entries(sharedData).map(([key, value]) => ({ key, value }))
        })
      }
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <Tabs defaultValue="info" className="space-y-6">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="contractors">Members</TabsTrigger>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setCompanyData(prev => ({ ...prev, paymentTerms: e.target.value }))}
                    placeholder="Net 30"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setCompanyData(prev => ({ ...prev, paymentAddress: e.target.value }))}
                  placeholder="Your wallet address for receiving payments"
                />
                <p className="text-xs text-gray-500 mt-1">
                  This address will be used as the default payment address for crypto invoices
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="shared">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Additional information that will be available to contractors when creating invoices
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-3">
                {Object.entries(sharedData).map(([key, value]) => (
                  <div key={key} className="flex gap-2">
                    ...
                    <Input
                      value={value}
                      onChange={(e) => setSharedData(prev => ({ ...prev, [key]: e.target.value }))}
              ...
              <Button
                variant="outline"
                onClick={() => setSharedData(prev => ({ ...prev, [`field_${Object.keys(prev).length + 1}`]: '' }))}
                className="w-full"
              >
          ...
          <div className="space-y-6">
                ...
                <Card>
                  <CardContent className="pt-6">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <p className="text-2xl font-bold">{membersData.stats.totalMembers}</p>
                ...
                <Card>
                  <CardContent className="pt-6">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <p className="text-2xl font-bold">{membersData.stats.totalInvites}</p>
                ...
                <Card>
                  <CardContent className="pt-6">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <p className="text-2xl font-bold">{membersData.stats.totalInviteUses}</p>
                        <p className="text-xs text-muted-foreground">Successful Joins</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  People who have joined your company
                </CardDescription>
              </CardHeader>
              <CardContent>
                          ...
                          <div className="flex items-center gap-2 mb-1">
                            <div className="flex flex-col">
                              <span className="font-medium text-sm">
                                {member.businessName || member.email || 'Unknown User'}
                              </span>
                              {member.email && (
            ...
            <Card>
              ...
              <CardContent>
                {inviteLinks.length ? (
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-sm font-mono">
                              {window.location.origin}/signin?invite={link.token.slice(0, 8)}...
                            </span>
                  ...
                  <p className="text-center text-gray-500 py-8">
                    No pending invitations. Create invite links to add contractors.
                  </p>
                )}
              </CardContent>
            </Card>
        ...
        <TabsContent value="invites">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Generate and manage links to invite contractors to join your company
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-200">
                <div>
                  <h3 className="font-medium text-blue-900">How Invite Links Work</h3>
                  <p className="text-sm text-blue-700 mt-1">
                    Share these links with contractors. When they sign in, they'll automatically join your company.
                  </p>
                </div>
                ...
                {inviteLinks.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 mb-4">
                      Create your first invite link to start adding contractors to your company.
                    </p>
                    <Button onClick={handleCreateInviteLink} disabled={!company}>
                      <Plus className="h-4 w-4 mr-2" />

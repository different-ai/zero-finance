...
export function SwapCard({ primarySafeAddress }: SwapCardProps) {
  ...
  const embeddedWallet = wallets.find((w) => w.walletClientType === 'privy' && w.chainId === `eip155:${base.id}`);
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      ...
      try {
        ...
        const balanceWei = await publicClient.getBalance({ address: embeddedWallet.address as Address });
  ...
  const handleInitiateSwap = async () => {
    ...
    try {
        ...
        const walletClient = createWalletClient({ account: fromAddress, chain: base, transport: custom(ethereumProvider) });
        const publicClient = createPublicClient({ chain: base, transport: http(BASE_RPC_URL) });
        ...
        const executeTxResponse = await safeSdk.executeTransaction(
            safeTransaction,
            { gasLimit: swapTxDataFromLifi.estimatedGas ? BigInt(swapTxDataFromLifi.estimatedGas) : undefined } // Use LI.FI gas if available
        );
        ...
        if (isTransactionHash(executeTxResponse)) {
            ...
            const potentialHash = txResponse.hash || txResponse.transactionResponse?.hash;
        ...
        if (txHash) {
            ...
            if (receipt.status === 'success') {
                toast.success(`Swap via Safe completed!`, { id: toastId, description: `Tx: ${receipt.transactionHash}` });
             ...
             toast.info('Swap via Safe executed (confirmation pending). Check your wallet.', { id: toastId });
        ...
        toast.error("Swap via Safe failed", { id: toastId, description: errorMessage });
        setSwapStatus(null);
    } finally {
    ...
    <div className="space-y-4">
         {!embeddedWallet && (
             <Alert variant="default" className="border-yellow-500/50 bg-yellow-50 text-yellow-800">
                ...
                <AlertTitle className="text-yellow-900">Embedded Wallet Not Found</AlertTitle>
                <AlertDescription className="text-yellow-700">
                  Could not find your Privy embedded wallet on the Base network. Please ensure it's set up.
                </AlertDescription>
            </Alert>
         )}
        ...
        {embeddedWallet && (
            <> 
             <div className="flex items-end space-x-2">
                <div className="flex-grow">
                    <label htmlFor="ethAmount" className="text-sm font-medium text-gray-700 mb-1 block">ETH Amount</label>
                    <Input 
                        id="ethAmount"
                        type="number"
                        placeholder="0.0" 
                 ...
                 <Button variant="outline" size="sm" onClick={handleMaxClick} disabled={!ethBalance || isLoadingBalance}>
                    MAX
                 </Button>
             </div>
             <p className="text-xs text-gray-500">
                {isLoadingBalance ? (
                    <span className="flex items-center"><Loader2 className="h-3 w-3 mr-1 animate-spin" /> Checking balance...</span>
                ) : (
            ...
            {swapError && (
             ...
             <p className="text-xs text-gray-500 text-center pt-2">
                Connected Wallet: <span className="font-mono break-all">{embeddedWallet.address}</span>
             </p>
            </>
        )}
    </div>

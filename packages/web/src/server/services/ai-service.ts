...
const classificationActionSchema = z.object({
  ...
  value: z
    .string()
    .nullable()
    .describe('Value for the action (e.g., category name)'),
});
...
const classificationResultSchema = z.object({
  ...
  confidence: z
    ...
    .min(0)
    .max(100)
    .describe('Confidence score for this match'),
  reason: z
    .string()
    .describe('Explanation of why this rule matched or did not match'),
  actions: z
    .array(classificationActionSchema)
    .describe('Actions to take if this rule matched'),
});

// Enhanced AI Schema for document processing
export const aiDocumentProcessSchema = z.object({
  documentType: z
    .enum(['invoice', 'receipt', 'payment_reminder', 'other_document']) // Added document type
    .describe(
      'The classified type of the document (e.g., invoice, receipt, payment_reminder, other_document).',
    ),
  aiRationale: z
    .string()
    .nullable() // Added field for LLM's rationale
    .describe(
      'A brief explanation from the AI about its categorization and key findings.',
    ),
  confidence: z
    .number()
    .min(0)
    .max(100)
    .describe(
      'Overall confidence score (0-100) of the categorization and extracted data. If confidence is below 60 for non-invoice types, or below 80 for invoices, treat with caution.',
    ),
  requiresAction: z
    .boolean()
    .describe(
      'Does this document clearly require an action from the user (e.g., payment, approval, response)? Set to true if an action is needed.',
    ),
  suggestedActionLabel: z
    .string()
    .nullable()
    .describe(
      "A concise label for the primary suggested action (e.g., 'Pay Invoice', 'Mark as Paid', 'Archive Receipt', 'Review Document'). Should align with requiresAction.",
    ),
  cardTitle: z
    .string()
    .describe(
      "A user-friendly, concise title for the inbox card. Should be descriptive and actionable (e.g., 'Amazon Order #123', 'Electricity Bill - Due Dec 15', 'Uber Receipt - $45.23'). Maximum 60 characters.",
    ),
  // Invoice specific fields (can be null if not an invoice or data not found)
  invoiceNumber: z
    .string()
    .nullable()
    .describe('Invoice reference number or ID'),
  buyerName: z
    .string()
    .nullable()
    .describe('Full name or business name of the buyer/customer'),
  sellerName: z
    .string()
    .nullable()
    .describe('Full name or business name of the seller/vendor'),
  amount: z
    .number()
    .nullable()
    .describe('Total amount due for the invoice or mentioned in the document'),
  ...
  issueDate: z
    ...
    .nullable()
    .describe(
      'Date the document was issued or pertains to, in YYYY-MM-DD format',
    ),
  items: z
    .array(
      z.object({
    ...
    .nullable()
    .describe('Line items if available'),
  extractedTitle: z
    .string()
    .nullable()
    .describe('The main title or heading from the document'),
  extractedSummary: z
    .string()
    .nullable()
    .describe("A brief summary of the document's content"),
  // Classification tracking - now structured
  classificationResults: z
    .array(classificationResultSchema)
    .nullable()
    .describe('Detailed results of classification rule evaluation'),
  shouldAutoApprove: z
    .boolean()
    .describe(
      'Whether this document should be auto-approved based on classification rules',
    ),
  shouldDismiss: z
    .boolean()
    .describe(
      'Whether this document should be auto-dismissed based on classification rules',
    ),
  shouldMarkPaid: z
    .boolean()
    .describe(
      'Whether this document should be marked as paid based on classification rules',
    ),
  suggestedCategories: z
    .array(z.string())
    .describe('Categories to apply based on classification rules'),
  expenseCategory: z
    .string()
    .nullable()
    .describe('Expense category to apply based on classification rules'),
});
...
For non-financial emails (introductions, newsletters, marketing, general correspondence), set documentType to "other_document" and provide minimal extraction.`;
    ...
    console.log(
      `[AI Service] Document classified as: ${processedDocument.documentType} with confidence: ${processedDocument.confidence}%`,
    );
    if (processedDocument.documentType === 'other_document') {
      console.log(
        `[AI Service] Non-financial document detected: ${processedDocument.extractedTitle || 'No title'}`,
      );
    }
...
export async function generateInvoiceFromText(
  ...
  try {
    ...
    if (!finalData.suggestedActionLabel)
      finalData.suggestedActionLabel = 'Process Invoice';

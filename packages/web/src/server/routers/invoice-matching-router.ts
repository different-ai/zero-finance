...
export const invoiceMatchingRouter = router({
        ...
        if (file.type === 'application/pdf') {
          ...
          const { object } = await generateObject({
            ...
            messages: [
              { 
                role: 'system', 
                content: 'Extract structured invoice data from the provided image.' 
              },
              {
                role: 'user',
                content: [
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        for (const row of rows) {
          ...
          const date = row.Date || row.date || row.TransactionDate || row.txn_date;
          ...
          const counterparty = row.Counterparty || row.counterparty || row.Vendor || row.vendor || '';
          const reference = row.Reference || row.reference || row.ID || row.id || '';
          ...
          transactions.push({
            ...
            amount: parseFloat(amount.toString().replace(/[^0-9.-]/g, '')).toString(),
            currency,
  ...
  proposeMatches: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        const matchedInvoiceIds = existingInvoiceMatches.map(m => m.invoiceId);
  ...
  getReconciliationData: protectedProcedure
    .query(async ({ ctx }) => {
      const userId = ctx.user.id;

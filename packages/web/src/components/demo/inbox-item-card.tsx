...
import { MoreHorizontal, HelpCircle, ArrowUpRight, ArrowDownLeft } from "lucide-react"
...
export function InboxItemCard({ item, onSelect, isHighlighted }: InboxItemCardProps) {
  const IconComponent = item.icon

  // Direction indicator
  const DirectionIcon = item.direction === "outbound" ? ArrowUpRight : ArrowDownLeft
  const directionColor = item.direction === "outbound" ? "text-blue-600" : "text-orange-600"
  const directionBg = item.direction === "outbound" ? "bg-blue-50" : "bg-orange-50"
  ...
  const getEntity = () => {
    if (item.direction === "outbound" && item.client) return `from ${item.client}`
  ...
  return (
    <Card
      className={cn(
        ...
        "border-l-4",
        isHighlighted ? "ring-2 ring-primary ring-offset-2 shadow-lg" : "shadow-sm",
      )}
      onClick={() => onSelect(item)}
    >
      <CardContent className="p-3 flex items-start space-x-3">
        <Checkbox id={`item-checkbox-${item.id}`} className="mt-1" aria-label={`Select item ${item.title}`} />

        {/* Direction indicator with icon */}
        <div className={cn("flex items-center justify-center w-8 h-8 rounded-full mt-0.5", directionBg)}>
          <DirectionIcon className={cn("h-4 w-4", directionColor)} />
        </div>

        <IconComponent
          className={cn("h-5 w-5 mt-1 flex-shrink-0", item.borderColorClass.replace("border-", "text-"))}
        />
        ...
        <div className="flex-grow overflow-hidden">
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge
              ...
              className={cn(
                ...
                item.direction === "outbound" ? "text-blue-700 border-blue-300" : "text-orange-700 border-orange-300",
              )}
            >
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            {formatAmount() && (
              <span
                className={cn("text-xs font-medium", item.direction === "outbound" ? "text-green-600" : "text-red-600")}
              >
                {formatAmount()}
              </span>
            )}
          ...
          {getEntity() && <p className="text-xs text-muted-foreground mb-2">{getEntity()}</p>}
        ...
        <div className="flex-shrink-0 flex flex-col items-end space-y-1 w-36 text-right">
          ...
          {item.source && (
            ...
            >
              {item.source.icon && <item.source.icon className="h-3 w-3 mr-1" />}

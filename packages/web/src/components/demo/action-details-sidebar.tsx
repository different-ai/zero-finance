...
export function ActionDetailsSidebar({ item, isOpen, onClose, showDemoControlsColumn }: ActionDetailsSidebarProps) {
    ...
    <AnimatePresence>
      {isOpen && item && (
        <motion.div
          ...
          style={{ right: typeof window !== "undefined" && window.innerWidth < 768 ? "0px" : rightOffset }}
        >
          {/* Header */}
          <div className="p-4 border-b flex items-start justify-between">
            <div className="flex items-start space-x-3">
              <IconComponent className="h-6 w-6 mt-1 text-primary flex-shrink-0" />
              <div>
          ...
          <ScrollArea className="flex-grow">
            <div className="p-4 space-y-6">
                ...
                <section>
                  <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
                    <div className="flex items-center mb-2">
                      ...
                      <h3 className="text-md font-semibold text-blue-700">{item.aiSuggestion.title}</h3>
                    </div>
                    <p className="text-sm text-blue-600 mb-3">{item.aiSuggestion.description}</p>
              ...
              {item.source && (
                <section>
                  <h3 className="text-xs uppercase text-muted-foreground mb-1 font-semibold">Source</h3>
                  <div className="flex items-center text-sm p-3 border rounded-md">
                    {item.source.icon ? (
                      <item.source.icon className="h-4 w-4 mr-2 text-muted-foreground" />
                    ) : (
                      <Mail className="h-4 w-4 mr-2 text-muted-foreground" />
                    )}
                    <span>{item.source.name}</span>
                    {item.source.details && (
                      <span className="text-muted-foreground ml-1 truncate">({item.source.details})</span>
                    )}
                  </div>
                </section>
              )}
              ...
              {item.chainOfThought && item.chainOfThought.length > 0 && (
                <section>
                  <h3 className="text-xs uppercase text-muted-foreground mb-3 font-semibold">Chain of Thought</h3>
                    ...
                    {item.chainOfThought.map((step, index) => (
                      <div key={step.id} className="relative pb-6">
                        ...
                        <div className="relative flex items-start space-x-3">
                          ...
                          <div className="min-w-0 flex-1 pt-0.5 text-sm">{step.text}</div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {item.impact && (
                <section>
                  <h3 className="text-xs uppercase text-muted-foreground mb-2 font-semibold">Impact</h3>
                  <div className="p-3 border rounded-md space-y-1 text-sm">
                    {item.impact.currentBalance !== undefined && (
                      <div className="flex justify-between">
                        <span>Current balance:</span>
                        <span>{formatCurrency(item.impact.currentBalance)}</span>
                      </div>
                    )}
                    {item.impact.postActionBalance !== undefined && (
                      <div className="flex justify-between font-medium">
                        <span>Post-action balance:</span>
                        <span>{formatCurrency(item.impact.postActionBalance)}</span>
                      </div>
                    )}
              ...
              {item.logInfo && (
                <section>
                  <h3 className="text-xs uppercase text-muted-foreground mb-2 font-semibold">Log Information</h3>
                  <div className="p-3 border rounded-md space-y-1 text-sm">
                    <div className="flex justify-between">
                      <span>ID:</span> <span className="font-mono text-xs">{item.logInfo.id}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Timestamp:</span>{" "}
                      <span>{format(new Date(item.logInfo.timestamp), "MMM d, yyyy, h:mm:ss a")}</span>
                    </div>
              ...
              <section>
                <h3 className="text-xs uppercase text-muted-foreground mb-2 font-semibold">Discussion</h3>

...
import { Loader2, CheckCircle, ClipboardCopy, CreditCard, AlertCircle } from 'lucide-react';
    ...
    return (
      <Card className="w-full bg-gradient-to-br from-red-50 to-red-100/40 border border-red-200/60 rounded-2xl p-6 shadow-sm">
        <CardHeader className="pb-2 text-center">
          ...
          <CardTitle className="text-lg font-semibold text-red-700">Error Loading Accounts</CardTitle>
        ...
        >
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between mb-1">
              <CardTitle className="text-xl font-bold text-gray-900">Virtual Bank Account</CardTitle>
              <Badge 
                variant="outline" 
                className={cn("capitalize text-xs px-2 py-0.5",
                  account.sourceAccountType === 'iban' ? "border-violet-300 bg-violet-50 text-violet-700" : "border-blue-300 bg-blue-50 text-blue-700"
                )}
              >
                {account.sourceAccountType === 'iban' ? 'IBAN (EUR)' : 'ACH (USD)'}
              </Badge>
            </div>
            <CardDescription className="text-sm text-gray-700">
          ...
          <CardContent className="pt-2">
            ...
            <div className="border-t border-gray-200 pt-4 mt-5">
              <h4 className="text-base font-semibold mb-2 text-gray-800">Destination Details</h4>
              <div className="space-y-2 text-sm bg-white/60 backdrop-blur-lg p-4 rounded-xl border border-gray-200">
                ...
                <div className="flex justify-between items-center pt-1">
                  ...
                  <span className={cn("font-mono text-xs truncate ml-2", account.destinationAddress ? "text-gray-700" : "text-gray-400")}>

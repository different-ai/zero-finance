...
export function TaxAccountSetupForm({
  onSetupComplete,
}: TaxAccountSetupFormProps) {
  ...
  const prepareCreateMutation =
    trpc.settings.userSafes.prepareCreate.useMutation({
      onSuccess: async (data) => {
        ...
        let smartWalletClient: Awaited<
          ReturnType<typeof getClientForChain>
        > | null = null;
        try {
          ...
          if (!smartWalletClient) {
            throw new Error(
              'Failed to get Privy Smart Wallet client for Base.',
            );
          }
          ...
          console.error(
            'Error getting Privy Smart Wallet client:',
            clientError,
          );
          toast.error(
            `Failed to initialize Smart Wallet: ${clientError.message}`,
          );
          setIsPreparing(false);
          setCreationStatus(null);
          return;
        ...
        try {
          ...
          if (connectedChainId !== `eip155:${base.id}`) {
            ...
            if (newChainId !== `eip155:${base.id}`) {
              toast.error(
                `Please switch to Base network (ID: ${base.id}) and try again.`,
                { id: toastId },
              );
              setIsPreparing(false);
              setCreationStatus(null);
          ...
          toast.loading(
            'Sending deployment transaction (sponsored)... Please wait',
            { id: toastId },
          );
          ...
          toast.success(
            `Deployment UserOperation submitted! Hash: ${txHash.slice(0, 10)}...`,
            { id: toastId },
          );
          toastId = toast.loading(
            'Waiting for network confirmation for Tax safe...',
          );
  ...
  const confirmCreateMutation =
    trpc.settings.userSafes.confirmCreate.useMutation({
      onSuccess: (data, variables) => {
        const explorerUrl = `https://basescan.org/tx/${variables.transactionHash}`;
        toast.success(
          <div className="flex flex-col">
            <span>
              {data.message || 'Tax Reserve account confirmed and saved!'}
            </span>
            <a
              href={explorerUrl}
              target="_blank"

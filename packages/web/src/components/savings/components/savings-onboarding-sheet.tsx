...
export default function SavingsOnboardingSheet({ open, onOpenChange, safeAddress }: SavingsOnboardingSheetProps) {
  const isMobile = useIsMobile()
  const { percentage, setPercentage, activateRule, isActivating } = useSavingsRule(safeAddress, 10)
  ...
  const SecurityPopover = () => (
    <Popover>
      ...
      <PopoverContent className="w-60 text-xs" align="end" style={{ color: "rgba(10, 10, 10, 0.9)" }}>
  ...
  const OnboardingContent = () => (
    <div className="flex flex-col items-center text-center p-6 pt-10 h-full max-w-[440px] mx-auto bg-[#F9FAFB]">
      ...
      <h1 className="text-[28px] font-semibold leading-snug" style={{ color: "rgba(10, 10, 10, 0.9)" }}>
        Automate Your Savings
      </h1>
      <p className="text-base font-normal mt-2 mb-8 leading-normal" style={{ color: "rgba(10, 10, 10, 0.9)" }}>
      ...
      <div className="w-full p-6 rounded-2xl bg-white mb-8 shadow-lg">
        <div className="flex items-center justify-around text-center">
          <div>
            <p className="text-xs uppercase" style={{ color: "rgba(10, 10, 10, 0.7)" }}>
              Deposit
            </p>
            <p className="text-[20px] font-semibold" style={{ color: "rgba(10, 10, 10, 0.9)" }}>
          ...
          <div>
            ...
            <p className="text-[20px] font-semibold" style={{ color: ACCENT_COLOR }}>
      ...
      <p className="text-sm font-medium mb-2" style={{ color: "rgba(10, 10, 10, 0.9)" }}>
        Save this percentage:
      </p>
      <div className="w-full mb-4">
        <AllocationSlider percentage={percentage} onPercentageChange={setPercentage} accentColor={ACCENT_COLOR} />
      </div>
        ...
        {presetPercentages.map((pct) => (
          <Button
            ...
            className={cn(
              ...
              percentage === pct ? "border-2 font-semibold" : "border-gray-300 text-gray-600",
            )}
            style={{
              borderColor: percentage === pct ? ACCENT_COLOR : "#E4E7EC",
              color: percentage === pct ? ACCENT_COLOR : "rgba(10, 10, 10, 0.8)",
            }}
          >
            {pct}%
          </Button>
      ...
      {percentage > 0 && (
        <div className="w-full p-4 rounded-2xl bg-white border mb-10 shadow-sm" style={{ borderColor: ACCENT_COLOR }}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium" style={{ color: "rgba(10, 10, 10, 0.9)" }}>
                First year earnings
              </p>
              <p className="text-3xl font-bold" style={{ color: "rgba(10, 10, 10, 0.9)" }}>
    ...
    <div className="p-6 text-center bg-[#F9FAFB]">
      <DrawerHeader className="p-0">
        <div className="flex justify-center mb-3">
          <CheckCircle className="w-10 h-10" style={{ color: POSITIVE_COLOR }} />
        </div>
        <DrawerTitle className="text-lg font-semibold" style={{ color: "rgba(10, 10, 10, 0.9)" }}>
          Rule is Ready to Go
        </DrawerTitle>
      </DrawerHeader>
      <p className="my-4 text-sm p-3 rounded-lg" style={{ color: "rgba(10, 10, 10, 0.9)", backgroundColor: "#EEF0FF" }}>
        <span className="font-bold" style={{ color: ACCENT_COLOR }}>
          {percentage}%
        </span>{" "}
        of every incoming deposit will route to the high-yield vault, earning ~{APY_RATE}% APY.
      </p>
  ...
  const mainContent = <div className="flex-1 overflow-y-auto bg-[#F9FAFB]">{<OnboardingContent />}</div>
      ...
      >
        {showConfetti && (
          <Player
            ...
            onEvent={(e: string) => e === "complete" && handleConfettiComplete()}
            keepLastFrame
          />
        )}
        {mainContent}

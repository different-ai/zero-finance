...
export default function AllocationSliderComponent({
  ...
  return (
    <div className="px-1 py-3">
      <style jsx global>{`
        .custom-themed-slider .relative.flex.w-full.touch-none.select-none.items-center {
          height: 1.25rem; /* Increased height for easier interaction */
        }
        .custom-themed-slider .relative.flex.w-full.touch-none.select-none.items-center > .absolute.h-full { /* Track */
        ...
        .custom-themed-slider .relative.flex.w-full.touch-none.select-none.items-center > span > .absolute.h-full { /* Range */
          background-color: ${accentColor} !important;
          border-radius: 0.125rem !important; /* Slightly rounded range */
        }
        .custom-themed-slider .relative.flex.w-full.touch-none.select-none.items-center > span > .block.h-5.w-5 { /* Thumb */
          height: 1.25rem !important; /* 20px */
          width: 1.25rem !important; /* 20px */
          background-color: white !important;
          border: 3px solid ${accentColor} !important;
          box-shadow: 0 2px 6px rgba(0,0,0,0.1), 0 0 0 3px white !important; /* Ring effect + subtle shadow */
          cursor: grab;
          transition: transform 0.1s ease-out;
        }
        .custom-themed-slider .relative.flex.w-full.touch-none.select-none.items-center > span > .block.h-5.w-5:active {
            cursor: grabbing;
            transform: scale(1.1);
        }
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Slider
              id={id}
              value={[percentage]}
              max={90} // Max percentage
              step={5} // Snap to 5% increments
              onValueChange={handleSliderChange}
              className="custom-themed-slider w-full"
            />
          </TooltipTrigger>
          <TooltipContent side="top" align="center" className="bg-deep-navy text-white text-xs rounded-md shadow-lg">

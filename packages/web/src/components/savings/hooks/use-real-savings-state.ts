...
export function useRealSavingsState(safeAddress: string | null, initialMainBalance: number) {
  ...
  useEffect(() => {
    if (safeAddress && config && state) {
      ...
      setSavingsState({
        ...
        apy: 4.96, // Default APY for now, we can add a separate APY query if needed
        firstEnabledAt: config.lastTrigger ? new Date(config.lastTrigger).getTime() : null,

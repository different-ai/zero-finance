...
import { MoreHorizontal, Copy, DollarSign, Settings, ArrowRightCircle } from "lucide-react"
...
export default function AccountCard({ initialBalance, safeAddress }: AccountCardProps) {
  const { savingsState, isLoading, mainBalance, optimisticAllocation } = useRealSavingsState(
    safeAddress,
    initialBalance,
  )
  ...
  return (
    <>
      <Card
        className={cn("w-full shadow-lg rounded-xl transition-colors duration-300", cardBackgroundColor, cardTextColor)}
      >
        <CardContent className="p-5 space-y-4">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              ...
              {isLoading && !savingsState ? (
                ...
                <h2 className="text-5xl font-bold text-current">{formatUsd(mainBalance)}</h2>
              )}
            </div>
            <div className="flex flex-col items-end gap-2">
              <Button variant="ghost" size="icon" className="h-8 w-8 text-current hover:bg-black/5">
          ...
          <div className="pt-2 space-y-1 border-t border-current/10">
            ...
            <p className="text-xs text-current/70 pl-[22px]">{projectionText}</p>
          </div>

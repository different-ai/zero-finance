...
export function InvoiceDisplay({
  ...
  const normalizedStatus =
    typeof invoiceData?.status === 'string'
  ...
  const showMarkAsPaid =
    canUpdateStatus && currentStatus !== 'paid' && invoiceData?.invoiceId;
  ...
  const formatCurrency = (
    ...
    if (amountStr.startsWith('0x')) {
      console.warn(
        `Invalid numeric value passed to formatCurrency: ${amountStr}`,
    ...
    try {
      ...
      if (isNaN(num)) {
        console.warn(
          `Invalid numeric value passed to formatCurrency (parsed as NaN): ${amountStr}`,
        ); // Optional: keep for debugging
      ...
      >
        ...
        <div className="text-right space-y-1">
          <div className="flex items-center justify-end gap-2">
            <span
              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${currentStatus === 'paid' ? 'bg-emerald-100 text-emerald-800' : currentStatus === 'pending' ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-700'}`}
            >
              {currentStatus
          ...
          <p className="text-sm text-gray-600">
            Issued: {formatDate(invoiceData.creationDate)}
          </p>
          {invoiceData.paymentTerms?.dueDate && (
            <p className="text-sm text-gray-600">
              Due: {formatDate(invoiceData.paymentTerms.dueDate)}
            </p>
          )}
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t pt-6">
          <div className="space-y-1">
            ...
            <p className="font-medium text-gray-800 leading-relaxed">
              {invoiceData.sellerInfo?.businessName || 'N/A'}
            </p>
            <p className="text-sm text-gray-600 leading-relaxed">
              {invoiceData.sellerInfo?.email || 'N/A'}
            </p>
            <p className="text-sm text-gray-600 leading-relaxed">
          ...
          <div className="space-y-1">
            ...
            <p className="font-medium text-gray-800 leading-relaxed">
              {invoiceData.buyerInfo?.businessName || 'N/A'}
            </p>
            <p className="text-sm text-gray-600 leading-relaxed">
              {invoiceData.buyerInfo?.email || 'N/A'}
            </p>
            <p className="text-sm text-gray-600 leading-relaxed">
        ...
        <div>
          <Table>
            <TableHeader className="bg-gray-100">
              <TableRow>
                <TableHead className="w-[50%] text-gray-600 px-3 py-3">
                  Description
                </TableHead>
                <TableHead className="text-right text-gray-600 px-3 py-3">
                  Qty
                </TableHead>
                <TableHead className="text-right text-gray-600 px-3 py-3">
                  Unit Price
              ...
              invoiceData.invoiceItems.length > 0 ? (
                invoiceData.invoiceItems.map((item, index) => (
                  <TableRow key={index} className="odd:bg-gray-50">
                    <TableCell className="font-medium text-gray-800 px-3 py-3">
                      {item.name || 'N/A'}
                    </TableCell>
                    <TableCell className="text-right text-gray-600 px-3 py-3">
                      {item.quantity || 'N/A'}
                    </TableCell>
                    <TableCell className="text-right text-gray-600 px-3 py-3">
                    ...
                    <TableCell className="text-right font-medium text-gray-800 px-3 py-3">
          ...
          {invoiceData.paymentType === 'fiat' &&
            ...
            !hideBankDetails && (
              <div>
                ...
                <p>
                  Account Holder:{' '}
                  {invoiceData.bankDetails.accountHolder || 'N/A'}
                </p>
                {invoiceData.bankDetails.iban && (
                  <p>IBAN: {invoiceData.bankDetails.iban}</p>
                )}
                ...
                {invoiceData.bankDetails.bic && (
                  <p>BIC/SWIFT: {invoiceData.bankDetails.bic}</p>
                )}
        ...
        {invoiceData.invoiceId && (
          <div className="border-t pt-4">
            <InvoiceAttachments invoiceId={invoiceData.invoiceId} />
          </div>
        )}
      </CardContent>
    </Card>

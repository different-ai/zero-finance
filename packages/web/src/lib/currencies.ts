...
/**
 * Currency types - native implementation (no Request Network dependency)
 */
...
export interface CryptoCurrencyConfig {
  type: CurrencyType.ETH | CurrencyType.ERC20;
...
export function getCurrencyConfig(
  ...
  if (!network) {
    ...
    if (identifier.startsWith('0x') && identifier.length === 42) {
      console.warn(
        `Network is required to get crypto currency config for address: ${identifier}`,
      );
    } else {
      // Maybe it was a crypto symbol without network?
      console.warn(
        `Network is required to get crypto currency config for symbol: ${identifier}`,
      );
    }
    return undefined;
  }
  ...
  if (lowerNetwork === 'base') {
    ...
    if (upperIdentifier === ETH_BASE_CONFIG.value.toUpperCase())
      return ETH_BASE_CONFIG; // Should not happen for ETH, but safe
    if (upperIdentifier === USDC_BASE_CONFIG.value.toUpperCase())
  ...
  console.warn(
    `Unsupported currency/network combination: ${identifier} on ${network}`,
  );
  return undefined;
}

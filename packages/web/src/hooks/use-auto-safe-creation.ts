...
export function useAutoSafeCreation() {
  ...
  const { smartWalletAddress, hasSmartWallet, createSmartWallet } =
    useSmartWallet();
  ...
  useEffect(() => {
    ...
    async function checkAndCreateSafe() {
      ...
      try {
        ...
        console.log(
          '0xHypr - Auto Safe Creation: Checking for existing Safe...',
        );

        // Check if user already has a primary safe
        const primarySafeAddr =
          await utils.settings.userSafes.getPrimarySafeAddress.fetch();

        if (primarySafeAddr) {
          console.log(
            '0xHypr - Auto Safe Creation: User already has a Safe:',
            primarySafeAddr,
          );
          return;
        }
        ...
        console.log(
          '0xHypr - Auto Safe Creation: No Safe found, creating one...',
        );

        // Step 1: Ensure smart wallet exists
        if (!hasSmartWallet || !smartWalletAddress) {
          console.log(
            '0xHypr - Auto Safe Creation: Creating smart wallet first...',
          );
          ...
          if (!smartWalletAddress) {
            console.error(
              '0xHypr - Auto Safe Creation: Failed to create smart wallet',
            );
            return;
          }
        }
        ...
        // Step 2: Create Safe
        console.log(
          '0xHypr - Auto Safe Creation: Creating Safe with smart wallet:',
          smartWalletAddress,
        );
        ...
        console.log(
          '0xHypr - Auto Safe Creation: Predicted Safe address:',

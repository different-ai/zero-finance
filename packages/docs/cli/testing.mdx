...
| Bank            | `zero bank proposals dismiss --proposal-id <id>`       | Failed  | Prod returns JSON parse error; route guard fix added.                                                      |
| Invoices        | `zero invoices create`                                 | Passed  | `invoice_id` 1c5ba4ae-3258-42ad-9804-ea196718d3bc.                                                         |
...
| Invoices        | `zero invoices send --invoice-id <id>`                 | Failed  | Prod returns JSON parse error; route guard + sender domain update added.                                   |
| Transactions    | `zero transactions list`                               | Failed  | Prod error `[object Object] is not valid JSON`; bank snapshot parse fix added.                             |
| Transactions    | `zero transactions get --transaction-id <id>`          | Failed  | Same bank snapshot parsing bug.                                                                            |
...
| Payment details | `zero payment-details share --recipient-email <email>` | Failed  | Resend domain not verified; updated sender to `payments@${AI_EMAIL_INBOUND_DOMAIN}`.                       |

...
<Tip>Use `finance --debug auth whoami` to print the resolved base URL.</Tip>
...
This opens a secure connection page on 0.finance and stores the API key for you. If the automatic handoff fails, the page shows a copyable key for manual paste.
...
<Tip>
  The base URL should be the root (e.g. `https://0.finance`), not `.../api/cli`.
  The CLI will normalize if you include `/api/cli`.
</Tip>

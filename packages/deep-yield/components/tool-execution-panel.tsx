...
// Type for tracking tool execution progress
    ...
    <AnimatePresence>
      {(activeTools.length > 0 || isStreaming) && (
        <motion.div
          ...
          className={cn(
            "fixed bottom-20 md:bottom-28 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-md",
            className
          )}
        >
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-lg shadow-lg border border-gray-200 dark:border-gray-800 px-4 py-3">
            <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
              {activeTools.length > 0 
                ? `Tools Running (${activeTools.length})` 
                : "AI is thinking..."}
            </h4>

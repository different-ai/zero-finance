...
export default function WiseDetectPage() {
  const router = useRouter();
  const [step, setStep] = useState<'idle' | 'detecting' | 'detected'>('idle');
  const { result: detectionResult, detectPayments, isProcessing: isDetecting } = usePaymentDetector('wise-detect-page');
  ...
  const handleSelect = useCallback((detection: typeof detections[0]) => {
    ...
    router.push(`/auto-pay/prepare/wise`);
  ...
  return (
    <main className="container max-w-2xl mx-auto p-4 space-y-4">
      <Card>
        <CardHeader>
          <CardTitle>Detect Wise Payments</CardTitle>
          <CardDescription>
            Scan your screen for payment-like content to process with Wise
          </CardDescription>
        </CardHeader>
        <CardContent>
